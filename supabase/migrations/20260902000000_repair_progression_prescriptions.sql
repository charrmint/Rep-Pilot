...
alter table public.progression_recommendations
  ...
  add column if not exists recommended_min_reps integer,
  add column if not exists recommended_max_reps integer,
  add column if not exists recommended_rir integer;

update public.progression_recommendations as recommendation
set
  recommended_min_reps = coalesce(
    recommendation.recommended_min_reps,
    session_exercise.min_reps
  ),
  recommended_max_reps = coalesce(
    recommendation.recommended_max_reps,
    session_exercise.max_reps
  ),
  recommended_rir = coalesce(recommendation.recommended_rir, 2)
...
alter table public.progression_recommendations
  drop constraint if exists progression_recommendations_rep_range_valid,
  drop constraint if exists progression_recommendations_rir_valid,
  drop constraint if exists progression_recommendations_prescription_complete;
...
create or replace function public.complete_workout_with_recommendations(
  p_session_id uuid,
